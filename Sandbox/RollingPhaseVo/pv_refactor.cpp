...
int setup_vocoder(PhaseVocoder pv)
{
    ...
    pv->stretched_length = pv->input_length ;
...
fail:
    // Plans 
    if (pv->p_r2c) fftwf_destroy_plan(pv->p_r2c);
    if (pv->p_c2r) fftwf_destroy_plan(pv->p_c2r);

    // FFTW-allocated buffers
    if (pv->time_buf) fftwf_free(pv->time_buf);
    if (pv->ifft_buf) fftwf_free(pv->ifft_buf);
    if (pv->X)        fftwf_free(pv->X);
    if (pv->Y)        fftwf_free(pv->Y);
    if (pv->win)      fftwf_free(pv->win);
...
void cleanup_vocoder(PhaseVocoder pv)
{
    ...
    // FFTW-allocated buffers
    if (pv->time_buf) { fftwf_free(pv->time_buf); pv->time_buf = NULL; }
    if (pv->ifft_buf) { fftwf_free(pv->ifft_buf); pv->ifft_buf = NULL; }
    if (pv->X)        { fftwf_free(pv->X);        pv->X = NULL; }
    if (pv->Y)        { fftwf_free(pv->Y);        pv->Y = NULL; }
    if (pv->win)      { fftwf_free(pv->win);      pv->win = NULL; }

    // malloc/calloc buffers
    free(pv->prev);     pv->prev = NULL;
    free(pv->sum);      pv->sum = NULL;
    free(pv->omega);    pv->omega = NULL;
    free(pv->stretched);pv->stretched = NULL;
    free(pv->norm);     pv->norm = NULL;
    free(pv->input);    pv->input = NULL;
...
size_t pv_process_ready(PhaseVocoder pv, int16_t* out, size_t out_cap) {
    ...
    while (_ring_used(pv->input_read, pv->input_write, input_length) >= (uint64_t)WINDOW_SIZE) {
        ...
        if (produced + (size_t)Hs > out_cap) break;
